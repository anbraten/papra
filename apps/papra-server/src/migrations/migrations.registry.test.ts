  ...
  describe('migrations', () => {
    ...
    test('regression test of the database state after running migrations, update the snapshot when the database state changes', async () => {
      ...
      expect(await serializeSchema({ db })).toMatchInlineSnapshot(`
        ...
        CREATE TABLE "custom_property_definitions" ( "id" text PRIMARY KEY NOT NULL, "created_at" integer NOT NULL, "updated_at" integer NOT NULL, "organization_id" text NOT NULL REFERENCES "organizations"("id") ON DELETE CASCADE ON UPDATE CASCADE, "name" text NOT NULL, "key" text NOT NULL, "description" text, "type" text NOT NULL, "display_order" integer NOT NULL DEFAULT 0 );
